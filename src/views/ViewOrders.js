...
export default function ViewOrders() {
    ...
    React.useEffect(() => {
        axios.post("http://localhost:8080/api/orders", {startIndex: 0}).then((res) => {
            setOrders(res.data)
        })
    }, [])
    ...
    const changeOrders = (e, dir) => {
        ...
        axios.post("http://localhost:8080/api/orders", {startIndex: startIndex}).then((res)=>{
            console.log(res)
                        ...
                        return (
                            <div className="flex py-4 relative text-white flex-col w-full border-b border-1 border-zinc-900">
                                ...
                                <div className="absolute right-1 bottom-1">
                                    <button onClick={(e) => {
                                        e.preventDefault()
                                        axios.delete("http://localhost:8080/api/deleteForm", {
                                            data: {
                                                val: element._id
                                            }
