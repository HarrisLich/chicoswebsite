...
export default function ViewAccount() {
    ...
    React.useEffect(() => {
        axios.post("http://localhost:80/api/accounts").then((res) => {
    ...
    const removeRefferal = (e, elem, event) => {
        ...
        axios.delete("http://localhost:80/api/deleteRefferal", {
            data: {
                user: elem._id,
                reffered: e
            }
    ...
    const markAccount = (event, elem) => {
        ...
        axios.post("http://localhost:80/api/updateAccount", elem).then((res) => {
            console.log(res)
            window.location.reload(true)
        })
    ...
    const showForm = (event, elem) => {
        ...
        axios.post("http://localhost:80/api/findForm", { username: elem.username }).then((res) => {
            console.log(res)
            setForm(res.data)
        })
    }
    ...
    const deleteAccount = (event, elem) => {
        event.preventDefault()
        axios.delete("http://localhost:80/api/deleteAccount", {
            data: {
